...
(async () => {
  ...
  app.get("/cars/", (req: Request, res: Response) => {
    ...
    let cars_list = cars
    //if we have an optional quary parameter,filter it
    if (make) {
  ...
  app.post("/cars/", (req: Request, res: Response) => {
    ...
    // destruct our body payload for our varibles
    let{make,type,model,cost,id} = req.body;


    // check to make sure all required varibles are set
    if (!id || !type || !model || !cost) {
      //repond wiyh an error if not
    ...
    // add this car to our local varible
    cars.push(new_car);

    //send the complete car obk=ject as a response 
